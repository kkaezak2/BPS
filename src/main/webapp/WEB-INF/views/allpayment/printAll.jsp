...
<link rel="stylesheet" type="text/css"
	href="/resources/css/allpayment_printAll.css">
<!-- js파일 -->
...
<body>
	...
	<div class="mt-5" id="div_body">
		...
		<!-- 증서번호 조회영역 시작 -->
		<div class="row justify-content-center" id="div_searchNumber">
			<form class="d-flex" action="/allpayment/printAll" method="post">
				<div class="col-1" id="divcol_status">
					<input class="form-check-input" type="radio" name="paymentStatus"
						id="optionsRadios1" value="정상" checked=""> <label
						class="form-check-label" for="optionsRadios1"> &nbsp;정상 </label>
				</div>

				<div class="col-1" id="divcol_status2">
					<input class="form-check-input" type="radio" name="paymentStatus"
						id="optionsRadios2" value="반송"> <label
						class="form-check-label" for="optionsRadios2"> &nbsp;반송 </label>
				</div>
		...
		<div class="row mt-5">
			<div class="col-11">
				<h3>
					<i class="fa-solid fa-stop"></i>&nbsp;납입내역조회
				</h3>
			</div>

			<div class="col-1 text-center">

				<c:set var="allpayment" value="${allPaymentList}" />
				<form action="
    <c:choose>
    <c:when 
    test="${not empty allpayment}">/excel/download/${allpayment[0].number}/${allpayment[0].paymentStatus}
    </c:when>
    <c:otherwise>
    #
    </c:otherwise>
    ...
    "
					method="get">
					<button type="submit" class="btn bg-dark text-white mt-3"
						data-bs-toggle="tooltip" data-bs-placement="bottom" title="엑셀다운로드"
						<c:if test="${empty allpayment}">disabled</c:if>>
						<i class="fa-solid fa-download" id="iconExcel"></i>
					</button>
				</form>
			</div>
		...
		<div class="allpayment-scroll">
			<c:forEach items="${allPaymentList}" var="allpayment">
				...
				<div class="row text-center justify-content-center">
					<div class="col custom-col">${allpayment.number}</div>
					<div class="col custom-col">${allpayment.contractor}</div>
