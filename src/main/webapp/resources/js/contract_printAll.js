...
function showAllRows(id, jsonData) {
	...
	let accountSelectDiv = $("<div class='col-2 account' id='div_account_clone'>");  // input요소들어갈 div생성
	...
	let countInput = $("<input id='count' readonly type='text' readonly>");  // 생성된 input DIV안에 input요소 추가
	let accountSelect = $('<select class="form-select border-secondary mt-2 text-center accountselect" id="accountId" style="height:30px;">' + accountSelectOptions);
...
// onblur: 포커스 해제 될 때 이벤트
function updateCount() {
   let startDateStr = $('#startDate').val();
   let endDateStr = $('#endDate').val();
   let finalDateStr = $('.custom-col.input_final_date').text().trim();

   if (startDateStr && endDateStr && finalDateStr) {
      ...
      if (finalDateStr) {
         ...
         $('#startDate').val(finalDate.toISOString().slice(0, 7));
         ...
         if (startDate <= endDate) {
            if (startDate >= finalDate) {
               ...
               // 시작년월이 종납년월보다 작은 경우에 알림창 띄우기
               //alert("시작년월은 종납년월 + 1개월 이어야 합니다.");
               
               // 시작년월이 종납년월보다 작은 경우에 스윗알럿으로 알림 표시
               Swal.fire({
                  icon: 'warning',
                  title: '납입기간 확인!',
                  text: '시작년월은 종납년월 + 1개월 이어야 합니다.',
                  confirmButtonColor: "#373a3c",
               });
               
               
               $('#count').val('');
               $('#endDate').val(''); // endDate 입력 필드 초기화
            }
         } else {
            // 날짜 범위가 잘못된 경우 처리
            $('#count').val('날짜 범위 오류');
